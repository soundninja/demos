...
(function($){
    ...
    smg.common = {
        init_ad_scrolling: function() {
            var $sidebarAd = $('.sidebar .sm-widget-ad').last();

            if(!$sidebarAd.length)
                return;

            var sidebarOffset = $sidebarAd.offset(),
                $boundingEl = $('.container.main-container'),
                boundingOffset = $boundingEl.offset();
            
            $sidebarAd.css('height', $sidebarAd.outerHeight());
            $sidebarAd.children().first().smgScrollAttach({
                $boundingEl: $('.sidebar'), 
                relativeBounds: {
                    top: $sidebarAd.position().top,
                    bottom: (sidebarOffset.top + $sidebarAd.outerHeight()) - (boundingOffset.top + $boundingEl.outerHeight())
                }
            });
        },
        ...
        init_social : function(holder, refresh, url, title) {
            ...
            var $socialHolder = $(holder), 
                ...
                sharrreDefaults = {
                    enableHover: false,
                    template: '<div><span class="icon-social"></span><span class="count">{total}</span></div>',
                    share: {},
                    enableTracking: true
                };
        ...
        init_social_scrolling: function() {
            // Init social scrolling
            var $socialIcons = $('.social-icons');

            if($socialIcons.length) {
                /*
                *   Base scroll attach options
                */
                var saOptions = {
                    $boundingEl: $('.article-body'), 
                    relativeBounds:{
                        top: $socialIcons.position().top
                    },
                    top_padding: 10
                };

                // If social icons are contracted then enable to toggling
                if($('.featured-embed-holder .js-main-image-slider--prev').length) {
                    var gallArrowTop = $('.js-main-image-slider--prev').offset().top,
                        open = false;

                    /*
                    * Expand/collapse gallery social icons
                    */
                    saOptions['scrollCallback'] = function(scrollAttach, dir, offset){
                        if(!open && dir == 'down' && offset.top > gallArrowTop) {
                            scrollAttach.calcBounds();
                            $socialIcons.removeClass('small');
                            scrollAttach.calcBounds();
                            open = true;
                        } else if(open && dir == 'up' && offset.top < gallArrowTop) {
                ...
                $socialIcons.smgScrollAttach(saOptions);
            }
        }
    };
    ...
    var scrollAttach = function($el, options){
        ...
        self.onScroll = function() {
            ...
            if(scrollTop > self.scrollTop) {
                // Scrolling down
                if(!(self.atBottom || self.atTop) && offset.bottom >= self.bounds.bottom) {
                    self.fixToBottom();
                } else if(!self.atBottom && scrollTop > offset.top) {
                    self.fixedScroll();
                }
            } else {
                // Scrolling up
                scrollDir = 'up';
                if(!(self.atBottom || self.atTop) && (offset.top + self.options.top_padding) <= self.bounds.top) {
                    self.fixToTop();
                }
                else if(!self.atTop && scrollTop < offset.top) {
                    self.fixedScroll();
                }
            }
