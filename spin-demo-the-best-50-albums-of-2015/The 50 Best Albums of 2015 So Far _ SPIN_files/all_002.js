...
(function($, sp, common){
    ...
    sp.common = {
        ...
        init: function() {
            ...
            var $headerCarousel = $('.header-carousel-reviews');
            if($headerCarousel.length > 0) {
                $headerCarousel.jcarousel({scroll: 1, itemVisibleInCallback: smg.sp.common.carouselPag});
            }
        ...
        carouselPag : function(carousel, activeItem, activeIndex) {
            var $carousel = $(activeItem).parents('.jcarousel-container');
            $carousel.find('.carousel-pagination li')
                .removeClass('active')
                .eq(activeIndex - 1)
                .addClass('active');
        },
        ...
        init_carousel_pagination : function() {
            ...
            $('li', $pagination).on('click', function() {
                // jcarousel is outdated and we dont have documentation.  Trying to simulate the time it takes for the slider to move
                var $carousel = $pagination.parents('.header-carousel');
                var $index = $(this).index();
                $pagination.parents('.header-carousel').jcarousel('scroll', ($index + 1));
                setTimeout(smg.sp.common.carouselPag, 1000, $carousel, $(this), $index + 1);
            });
        }
    };

    $(document).ready(function(){ sp.common.init(); });
    // $(window).load(function(){ smg.common.init_ad_scrolling(); });
    ...
    var pluginName = 'sharrre',
        ...
        popup = {
            ...
            twitter    : function (opt) {
                // Short url is optional so append utm to base url too
                var url = (opt.url !== '' ? opt.url : opt.buttons.twitter.url),
                    short_url = opt.shortUrl,
                    share_url;
                url = addURLParam(url, "utm_source", opt.utmSource);
                url = addURLParam(url, "utm_medium", "button");
                share_url = (short_url !== '' ? short_url : url);
