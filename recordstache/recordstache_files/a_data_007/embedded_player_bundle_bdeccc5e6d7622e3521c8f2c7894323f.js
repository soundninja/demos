    ...
    HEP.init = function(playerdata, parentpage, params) {
            Sound.whenloaded( function(status) {
                if(playerdata && status.success) {
                    ...
                    } else if (params['facebook.com']) {
                        // facebook instantiates a player only after the user has clicked
                        // something that looks like a play button, so go ahead and play it
                        playlist.play();
                    }
                } else if(!playerdata) {
